...
class Certificate {
	...
	public function store_certificate_image() {
		...
		// Validate the actual uploaded bytes; never trust the browser supplied MIME type.
		if ( ! isset( $_FILES['certificate_image'] ) || ! is_array( $_FILES['certificate_image'] ) ) {
			wp_send_json_error( array( 'message' => __( 'Certificate Image Error', 'tutor-pro' ) ), 400 );
		}
		$file = $_FILES['certificate_image']; // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
		$tmp  = isset( $file['tmp_name'] ) ? (string) $file['tmp_name'] : '';
		$size = isset( $file['size'] ) ? absint( $file['size'] ) : 0;
		if ( ! empty( $file['error'] ) || ! $tmp || ! is_uploaded_file( $tmp ) || $size < 1 || $size > 10485760 || 'image/jpeg' !== wp_get_image_mime( $tmp ) ) {
			wp_send_json_error( array( 'message' => __( 'Certificate Image Error', 'tutor-pro' ) ), 400 );
		}
		...
		if ( ! move_uploaded_file( $tmp, $file_dest ) ) { // phpcs:ignore WordPress.PHP.NoSilencedErrors.Discouraged
			wp_send_json_error( array( 'message' => __( 'Certificate Image Error', 'tutor-pro' ) ), 500 );
		}
		@chmod( $file_dest, 0640 );
	...
	public function get_user_certificates( int $user_id ): array {
		...
		foreach ( $certificates as $certificate ) {
			...
			$revoked         = '1' === (string) get_comment_meta( (int) $certificate->certificate_id, '_qalam_certificate_revoked', true );
			$certificate_url = $revoked ? '' : $this->get_certificate( $course_id );
			if ( ( ! $certificate_url && ! $revoked ) || $this->disable_certificate_for_individual_courses( $course_id ) ) {
				continue;
			}
