...
class Events {
	...
	public function ajax_google_meet_meeting_details() {
		...
		if ( ! $meeting || 'tutor-google-meet' !== $meeting->post_type || (int) $meeting->post_parent !== $topic_id || ! Validator::can_manage_meeting( $meeting_id ) ) {
			$this->json_response(
	...
	public function create_meeting() {
		...
		if ( $is_update ) {
			$target_post_id = absint( $post['post-id'] );
			if ( ! Validator::can_manage_meeting( $target_post_id ) ) { $this->json_response( tutor_utils()->error_message(), null, HttpHelper::STATUS_FORBIDDEN ); }
			$stored = json_decode( (string) get_post_meta( $target_post_id, EventsModel::POST_META_KEYS[2], true ) );
			if ( ! $stored || empty( $stored->id ) || empty( $post['event-id'] ) || ! hash_equals( (string) $stored->id, (string) $post['event-id'] ) ) { $this->json_response( __( 'Invalid meeting id', 'tutor-pro' ), null, HttpHelper::STATUS_BAD_REQUEST ); }
		}
	...
	public function delete_meeting() {
		...
		$event_id       = sanitize_text_field( (string) Input::post( 'event-id' ) );
		...
		if ( ! Validator::can_manage_meeting( $post_id ) ) {
			$this->json_response( tutor_utils()->error_message(), null, HttpHelper::STATUS_FORBIDDEN );
		}
		$stored = json_decode( (string) get_post_meta( $post_id, EventsModel::POST_META_KEYS[2], true ) );
		if ( ! $stored || empty( $stored->id ) || ! hash_equals( (string) $stored->id, (string) $event_id ) ) {
			$this->json_response( __( 'Invalid meeting id', 'tutor-pro' ), null, HttpHelper::STATUS_BAD_REQUEST );
		}
	...
	public function reset_credential() {
		...
		if ( $this->google_client->reset_credentials() ) {
