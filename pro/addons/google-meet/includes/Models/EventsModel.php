...
class EventsModel {
	...
	public static function get( string $context, array $sorting_args, array $paging_args, $only_course_meeting = false ): array {
		...
		$context            = $context;
		$course_id          = absint( $sorting_args['course_id'] ?? 0 );
		$search_term        = sanitize_text_field( (string) ( $sorting_args['search_term'] ?? '' ) );
		$author_id          = $sorting_args['author_id'] ?? '';
		$date               = sanitize_text_field( (string) ( $sorting_args['date'] ?? '' ) );
		$order              = 'DESC' === strtoupper( (string) ( $sorting_args['order'] ?? 'ASC' ) ) ? 'DESC' : 'ASC';
		$limit              = max( 1, absint( $paging_args['limit'] ?? 10 ) );
		$offset             = max( 0, absint( $paging_args['offset'] ?? 0 ) );
		...
		if ( '' !== $author_id && null !== $author_id ) {
			$author_ids = is_array( $author_id ) ? $author_id : preg_split( '/\s*,\s*/', (string) $author_id );
			$author_ids = array_values( array_filter( array_map( 'absint', (array) $author_ids ) ) );
			if ( $author_ids ) {
				$author_clause = 'AND meeting.post_author IN ( ' . QueryHelper::prepare_in_clause( $author_ids ) . ' )';
			}
		}

		$date_clause = '';
		if ( '' !== $date && preg_match( '/^\d{4}-\d{2}-\d{2}$/', $date ) ) {
			$escaped_date = $wpdb->prepare( '%s', $date );
			$date_clause  = "AND ( DATE(start_date.meta_value) = CAST( {$escaped_date} AS DATE ) OR DATE(end_date.meta_value) = CAST( {$escaped_date} AS DATE ) )";
		}
		...
		if ( $meeting_parent_ids ) {
			$parent_ids = array_values( array_filter( array_map( 'absint', is_array( $meeting_parent_ids ) ? $meeting_parent_ids : array( $meeting_parent_ids ) ) ) );
			if ( $parent_ids ) {
