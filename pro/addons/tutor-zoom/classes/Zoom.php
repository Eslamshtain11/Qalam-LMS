...
class Zoom extends Tutor_Base {
	...
	public function ajax_zoom_meeting_details() {
		...
		if ( ! $post || $this->zoom_meeting_post_type !== $post->post_type || (int) $post->post_parent !== $topic_id ) {
			$this->json_response(
	...
	public function tutor_zoom_save_meeting() {
		...
		$course = get_post( $course_id );
		if ( ! $course || tutor()->course_post_type !== $course->post_type || ! tutor_utils()->can_user_manage( 'course', $course_id ) ) {
			$this->json_response( tutor_utils()->error_message(), null, HttpHelper::STATUS_FORBIDDEN );
		}
		if ( $topic_id ) {
			$topic = get_post( $topic_id );
			if ( ! $topic || tutor()->topics_post_type !== $topic->post_type || (int) $topic->post_parent !== $course_id || ! tutor_utils()->can_user_manage( 'topic', $topic_id ) ) {
				$this->json_response( tutor_utils()->error_message(), null, HttpHelper::STATUS_FORBIDDEN );
			}
		}
		if ( $meeting_id ) {
			$meeting_post = get_post( $meeting_id );
			$expected_parent = $topic_id ? $topic_id : $course_id;
			$meeting_course = (int) tutor_utils()->get_course_id_by_content( $meeting_id );
			if ( ! $meeting_post || $this->zoom_meeting_post_type !== $meeting_post->post_type || (int) $meeting_post->post_parent !== $expected_parent || $meeting_course !== $course_id ) {
				$this->json_response( tutor_utils()->error_message(), null, HttpHelper::STATUS_FORBIDDEN );
			}
		}
	...
	public function tutor_zoom_delete_meeting() {
		...
		$post_id = Input::post( 'meeting_id', 0, Input::TYPE_INT );
		$meeting_post = $post_id ? get_post( $post_id ) : null;
		if ( ! $meeting_post || $this->zoom_meeting_post_type !== $meeting_post->post_type ) {
		...
		if ( (int) tutor_utils()->get_course_id_by_content( $post_id ) !== (int) $course_id ) {
			$this->response_bad_request( tutor_utils()->error_message() );
		}
	...
	public function get_meetings( $limit = 10, $page = 1, $context = '', $args = array(), $get_from_topic = true, $meeting_id = null ) {
		...
		// SQL fragments below are intentionally restricted to canonical values before concatenation.
		$limit    = null === $limit ? null : max( 1, absint( $limit ) );
		$page     = max( 1, absint( $page ) );
		$order    = 'ASC' === strtoupper( (string) ( $args['order'] ?? 'DESC' ) ) ? 'ASC' : 'DESC';
		$order_by = '_meeting.post_title' === (string) ( $args['order_by'] ?? '' ) ? '_meeting.post_title' : '_meta_start.meta_value';

		$limit_offset  = ' ORDER BY ' . $order_by . ' ' . $order;
		$limit_offset .= null !== $limit ? ' LIMIT ' . $limit . ' OFFSET ' . ( ( $page - 1 ) * $limit ) : '';

		// Get meetings by id. Never allow caller-provided strings into an IN clause.
		if ( $meeting_id ) {
			$meeting_ids = array_values( array_filter( array_map( 'absint', is_array( $meeting_id ) ? $meeting_id : array( $meeting_id ) ) ) );
			if ( $meeting_ids ) {
				$filter_clause .= ' AND _meeting.ID IN(' . QueryHelper::prepare_in_clause( $meeting_ids ) . ')';
			}
		}
		...
		if ( ! empty( $args['course_id'] ) ) {
			...
			$parent_ids = array_values( array_filter( array_map( 'absint', array_merge( $topic_ids, array( absint( $args['course_id'] ) ) ) ) ) );
			if ( $parent_ids ) {
				$filter_clause .= ' AND _meeting.post_parent IN (' . QueryHelper::prepare_in_clause( $parent_ids ) . ')';
			}
		}

		// Author filter.
		if ( ! empty( $args['author'] ) ) {
			$filter_clause .= ' AND _meeting.post_author=' . absint( $args['author'] );
		}

		// Search filter.
		if ( ! empty( $args['search'] ) ) {
			$filter_clause .= $wpdb->prepare( ' AND _meeting.post_title LIKE %s', '%' . $wpdb->esc_like( sanitize_text_field( (string) $args['search'] ) ) . '%' );
		}

		// Date filter. Accept only a canonical YYYY-MM-DD value.
		if ( ! empty( $args['date'] ) ) {
			$date = sanitize_text_field( (string) $args['date'] );
			if ( preg_match( '/^\d{4}-\d{2}-\d{2}$/', $date ) ) {
				$filter_clause .= $wpdb->prepare( ' AND DATE(_meta_start.meta_value)=%s', $date );
			}
		}
		...
		if ( ! empty( $args['post_parent'] ) ) {
			$parent_ids = array_values( array_filter( array_map( 'absint', is_array( $args['post_parent'] ) ? $args['post_parent'] : array( $args['post_parent'] ) ) ) );
			if ( $parent_ids ) {
				$prepare_in_clause     = QueryHelper::prepare_in_clause( $parent_ids );
