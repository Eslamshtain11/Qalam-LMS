...
if ( qalam_220_is_admin_surface_context( 'zoom' ) ) {
	$filters = array(
		'bulk_action' => false,
		'filters'     => array(
			array(
		...
		<div class="tutor-wp-dashboard-filter-items tutor-d-flex tutor-flex-xl-nowrap tutor-flex-wrap tutor-justify-between">
			...
			<div class="tutor-wp-dashboard-filter-item tutor-col tutor-my-lg-0 tutor-my-12">
				...
				<select name="course-id" class="tutor-zoom-course tutor-form-select" data-searchable style="width:100%; max-width:100%;">
					...
					<?php
					$courses = \TUTOR\User::is_admin() ? CourseModel::get_courses() : CourseModel::get_courses_by_instructor();
					if ( ! empty( $courses ) ) {
...
<div class="<?php echo qalam_220_is_admin_surface_context( 'zoom' ) ? 'tutor-mt-24' : ''; ?>">
	<?php
	if ( ! empty( $meetings ) ) {
		...
		echo qalam_220_is_admin_surface_context( 'zoom' ) ? '<div class="tutor-admin-container tutor-admin-container-lg">' : '';
		include dirname( __DIR__ ) . '/template/meeting-list.php';
		echo qalam_220_is_admin_surface_context( 'zoom' ) ? '</div>' : '';
	} else {
		$empty_state_class = qalam_220_is_admin_surface_context( 'zoom' ) ? 'tutor-admin-container tutor-admin-container-lg' : 'tutor-mt-24';
		?>
	...
	if ( count( $meetings ) ) {
		...
		if ( $total_items > $per_page ) {
			...
			?>
			<div class="<?php echo qalam_220_is_admin_surface_context( 'zoom' ) ? 'tutor-admin-container tutor-admin-container-lg tutor-mt-48' : ''; ?>">
