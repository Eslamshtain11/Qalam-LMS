...
class EmailNotification {
	...
	public function feedback_submitted_for_quiz_attempt( $attempt_id ) {
		...
		$instructor_feedback = '';
		$attempt_info        = isset( $attempt->attempt_info ) ? unserialize( $attempt->attempt_info, array( 'allowed_classes' => false ) ) : false;
		if ( $attempt_info ) {
		...
		$replacable['{review_url}']           = sprintf(
			"<a href='%s'>%s</a>",
			ProQuizModel::get_attempt_details_url( $attempt_id, 'student' ),
			ProQuizModel::get_attempt_details_url( $attempt_id, 'student' )
		);
		$replacable['{quiz_review_url}']      = ProQuizModel::get_attempt_details_url( $attempt_id, 'student' );
	...
	public function send_mail( $mails ) {
		...
		foreach ( $mails as $mail ) {
			$mail['headers'] = unserialize( $mail['headers'], array( 'allowed_classes' => false ) );
