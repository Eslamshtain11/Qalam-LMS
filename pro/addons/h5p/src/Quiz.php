...
class Quiz extends Tutor_Base {
	...
	private function save_h5p_quiz_statement_results( $result_statement, $quiz_id, $attempt_id, $question_id, $content_id ) {
		...
		$server_results = Utils::addon_config()->h5p_admin_plugin->get_results( $content_id, get_current_user_id(), 0, 10, 1, 0 );
		$content_result = is_array( $server_results ) && ! empty( $server_results[0] ) ? $server_results[0] : null;
		// SECURITY: official Tutor grade MUST come from H5P's server-persisted result, never the client xAPI score.
		if ( ! is_object( $content_result ) || ! isset( $content_result->score, $content_result->max_score ) || (float) $content_result->max_score <= 0 ) {
			return 0; // fail closed: keep xAPI analytics, but do not create an official score.
		}
		$raw=max(0,(float)$content_result->score); $max=max(0.000001,(float)$content_result->max_score); $scaled=min(1,max(0,$raw/$max));
		$quiz_result['max_score']=(int)round($max); $quiz_result['min_score']=0; $quiz_result['raw_score']=(int)round($raw); $quiz_result['scaled_score']=(int)round($scaled);
		$quiz_result['completion'] = ! empty( $content_result->finished );
		$quiz_result['success']    = $raw >= $max;
		$quiz_result['response']   = isset( $result_statement->response ) ? $result_statement->response : '';
		$quiz_result['opened']     = (int)($content_result->opened??0);
		$quiz_result['finished']   = (int)($content_result->finished??0);
	...
	/**
	 * Bind an H5P score request to the current user's active Tutor attempt and canonical H5P question.
	 *
	 * @return object|false Question object when the tuple is valid.
	 */
	private function validate_h5p_attempt_tuple( int $quiz_id, int $attempt_id, int $question_id, int $content_id ) {
		$user_id = get_current_user_id();
		$attempt = $attempt_id ? tutor_utils()->get_attempt( $attempt_id ) : null;
		if ( ! $user_id || ! is_object( $attempt ) || (int) ( $attempt->user_id ?? 0 ) !== $user_id || (int) ( $attempt->quiz_id ?? 0 ) !== $quiz_id || \Tutor\Models\QuizModel::ATTEMPT_STARTED !== (string) ( $attempt->attempt_status ?? '' ) ) {
			return false;
		}
		$question = \Tutor\Models\QuizModel::get_quiz_question_by_id( $question_id );
		if ( ! is_object( $question ) || (int) ( $question->quiz_id ?? 0 ) !== $quiz_id || 'h5p' !== (string) ( $question->question_type ?? '' ) || absint( $question->question_description ?? 0 ) !== $content_id ) {
			return false;
		}
		return $question;
	}
	...
	public function save_h5p_question_xAPI_statement() {
		...
		if ( ! $this->validate_h5p_attempt_tuple( $quiz_id, $attempt_id, $question_id, $content_id ) ) {
			wp_send_json_error( array( 'message' => __( 'Invalid H5P quiz attempt.', 'tutor-pro' ) ), 403 );
		}
		$course_id           = tutor_utils()->get_course_id_by_subcontent( $quiz_id );
		...
		if ( isset( $statement->result ) ) {
			...
			$trusted_result = $quiz_result_id ? $wpdb->get_row( $wpdb->prepare( "SELECT raw_score,max_score,min_score,scaled_score FROM {$wpdb->prefix}tutor_h5p_quiz_result WHERE result_id=%d", $quiz_result_id ) ) : null;
			if ( $trusted_result ) {
				$quiz_xapi_statement['result_max_score']=(int)$trusted_result->max_score; $quiz_xapi_statement['result_min_score']=(int)$trusted_result->min_score; $quiz_xapi_statement['result_raw_score']=(int)$trusted_result->raw_score; $quiz_xapi_statement['result_scaled_score']=(int)$trusted_result->scaled_score;
	...
	public function check_h5p_question_answered() {
		...
		$attempt = $attempt_id ? tutor_utils()->get_attempt( $attempt_id ) : null;
		if ( ! $user_id || ! is_object( $attempt ) || (int) ( $attempt->user_id ?? 0 ) !== $user_id || (int) ( $attempt->quiz_id ?? 0 ) !== $quiz_id ) {
			wp_send_json_error( array( 'message' => __( 'Invalid H5P quiz attempt.', 'tutor-pro' ) ), 403 );
		}
		...
		if ( is_array( $question_ids ) && count( $question_ids ) ) {
			foreach ( $question_ids as $id_data ) {
				...
				if ( 0 !== $question_id && 0 !== $content_id && $this->validate_h5p_attempt_tuple( $quiz_id, $attempt_id, $question_id, $content_id ) ) {
	...
	public function delete_h5p_quiz_result_by_attempt_id( string $attempt_ids ) {
		global $wpdb;
		$ids = array_values( array_unique( array_filter( array_map( 'absint', preg_split( '/\s*,\s*/', $attempt_ids ) ) ) ) );
		if ( ! $ids ) { return; }
		$placeholders = implode( ',', array_fill( 0, count( $ids ), '%d' ) );
		$sql = $wpdb->prepare( "DELETE FROM {$wpdb->prefix}tutor_h5p_quiz_result WHERE attempt_id IN ($placeholders)", $ids ); // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
