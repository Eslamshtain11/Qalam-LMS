...
<div class="tutor-course-topic-single-body">
	<div class="tutor-quiz-wrapper tutor-d-flex tutor-justify-center tutor-mt-36 tutor-pb-80">
		<div id="tutor-assignment-wrap" class="tutor-quiz-wrap tutor-course-assignment-details tutor-submit-assignment  tutor-assignment-result-pending">
			...
			<?php
			if ( ( $is_submitting || isset( $_GET['update-assignment'] ) ) && ( $remaining_time > $now || 0 === $time_value || $is_retry_allowed ) && ! $is_expired ) :
				if ( get_the_content() ) {
					tutor_load_template_from_custom_path(
						$assignment_path . 'templates/details.php'
					);
								...
								<div class="tutor-assignment-attachment tutor-mt-32 tutor-py-20 tutor-px-16 tutor-py-sm-32 tutor-px-sm-32">
											...
											<form>
												<label for="tutor-assignment-file-upload">
													<input type="file" id="tutor-assignment-file-upload"
														name="attached_assignment_files[]" multiple accept="image/*,.pdf,.doc,.docx,.xls,.xlsx,.ppt,.pptx,.txt">
									...
									<div class="tutor-container tutor-pt-16 tutor-update-assignment-attachments">
										<div class="tutor-row tutor-gy-3" id="tutor-student-assignment-edit-file-preview">
											<?php
												$attachments = AssignmentSubmissionSecurity::get_attachments( absint( $assignment_comment_id ), true );
													...
													<?php foreach ( $attachments as $attachment ) : ?>
														...
														<div class="tutor-instructor-card tutor-col-sm-5 tutor-py-16 tutor-mr-16">
															...
															<div class="tutor-icard-content">
																...
																<div class="tutor-fs-6 tutor-color-secondary"><?php echo esc_html( $attachment->name ); ?></div>

																<div class="tutor-fs-7"><?php echo esc_html( tutor_utils()->get_readable_filesize( AssignmentSubmissionSecurity::size( $attachment, absint( $assignment_comment_id ) ) ) ); ?></div>

															</div>

															<div class="tutor-attachment-file-close tutor-d-flex tutor-align-center">

																<a class="tutor-iconic-btn tutor-iconic-btn-outline" href="<?php echo esc_url( $attachment->url ); ?>" data-id="<?php echo esc_attr( $assignment_comment_id ); ?>" data-name="<?php echo esc_attr( $attachment->name ); ?>" target="_blank" rel="noopener noreferrer"><span class="tutor-icon-times"></span></a>
						...
						<div class="tutor-assignment-footer-btn tutor-d-flex">
							<?php if ( Assignments::is_resubmission_allowed( $post_id, $user_id, $submitted_assignment ) ) : ?>
								<form action="" method="post" id="tutor_assignment_start_form">
									...
									<input type="hidden" name="assignment_id" value="<?php echo esc_attr( get_the_ID() ); ?>">
									<button type="submit" id="tutor_assignment_start_btn" class="tutor-btn tutor-btn-primary tutor-mr-16"<?php echo $has_reached_maximum_attempts || $is_expired ? ' disabled' : ''; ?>>
