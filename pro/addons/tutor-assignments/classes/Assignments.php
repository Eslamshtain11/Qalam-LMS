...
use Qalam\Security\AssignmentSubmissionSecurity;
...
class Assignments extends Tutor_Base {
	...
	public function insert_assignment_submit( $store_data ) {
		...
		$upload_attachment = $this->handle_assignment_attachment_uploads( $assignment_id, $assignment_submit_id );
	...
	public function update_assignment_submit( $store_data ) {
		...
		$upload_attachment = $this->handle_assignment_attachment_uploads( $assignment_id, $update_id ? $update_id : $assignment_submit_id );
	 ...
	 * @return array
	 */
	public function handle_assignment_attachment_uploads( $assignment_id = 0, $submission_id = 0 ) {
		...
		$allow_to_upload = max( 0, (int) tutor_utils()->get_assignment_option( $assignment_id, 'upload_files_limit' ) );
		if ( empty( $_FILES['attached_assignment_files'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Missing -- caller verifies Tutor nonce.
			return $attached_files;
		}

		$files = $_FILES['attached_assignment_files']; // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized -- validated as uploads below.
		$names = is_array( $files['name'] ?? null ) ? $files['name'] : array();
		if ( $allow_to_upload > 0 && count( array_filter( $names ) ) > $allow_to_upload ) {
			wp_die( esc_html__( 'Maximum attachment count exceeded.', 'tutor-pro' ), '', array( 'response' => 400 ) );
		}

		$max_size_mb = max( 1, (int) tutor_utils()->get_assignment_option( $assignment_id, 'upload_file_size_limit', 2 ) );
		...
		foreach ( $names as $key => $value ) {
			...
			$file = array(
				...
				'size'     => absint( $files['size'][ $key ] ?? 0 ),
			);
			if ( $file['size'] > $max_bytes ) {
				wp_die( esc_html( sprintf( __( 'Maximum attachment upload size allowed is %d MB', 'tutor-pro' ), $max_size_mb ) ), '', array( 'response' => 400 ) );
			}
			$stored = AssignmentSubmissionSecurity::store_upload( $file, $assignment_id, $submission_id, $owner_id );
			if ( is_wp_error( $stored ) ) {
				wp_die( esc_html( $stored->get_error_message() ), '', array( 'response' => 400 ) );
			}
			$attached_files[] = $stored;
		}
	...
	private function delete_assignment_submission( $submitted_id ) {
		...
		// Delete assignment attachments from Qalam's private store.
		AssignmentSubmissionSecurity::delete_submission_files( absint( $submitted_id ) );
	...
	public function remove_assignment_attachment(): void {
		...
		$is_owner   = $user_id === (int) $assignment->user_id;
		$can_manage = tutor_utils()->can_user_edit_course( $user_id, $course_id );
	...
	public static function delete_attachment( $assignment_comment_id, $file_name ) {
		return AssignmentSubmissionSecurity::delete_file( absint( $assignment_comment_id ), sanitize_file_name( (string) $file_name ) );
	}
	...
	public static function render_sidebar_nav( WP_Post $assignment, $course_id, $can_access, $tutor_current_content_id ) {
		...
		tutor_load_template(
			...
			array(
				'item'         => $assignment,
				'active'       => $tutor_current_content_id === $assignment->ID,
				'can_access'   => $can_access,
				'is_completed' => false,
				'type_label'   => __( 'Assignment', 'tutor-pro' ),
				'icon'         => $icon_name,
				'status_class' => $assignment_status,
			)
		);
	}
