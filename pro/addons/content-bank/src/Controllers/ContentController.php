...
class ContentController extends BaseController {
	...
	public function ajax_get_content_bank_contents() {
		...
		foreach ( $content_types as $content_type ) {
			$post_type = ContentModel::get_post_type_by_content_type( $content_type );
			$post_types[] = $post_type;
		}

		$excluded_post_types = array();
		if ( ! Helper::is_assignment_addon_enabled() ) {
			$excluded_post_types[] = ContentModel::ASSIGNMENT_POST_TYPE;
		}

		if ( tutor_utils()->count( $post_types ) ) {
			$where['post_type'] = $post_types;
		} elseif ( tutor_utils()->count( $excluded_post_types ) ) {
			$where['post_type'] = array( 'NOT IN', array_values( array_unique( $excluded_post_types ) ) );
		}
