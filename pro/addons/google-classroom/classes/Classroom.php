...
class Classroom {
	...
	public function __construct( $owner_id = null, $post_id = null, $ret = false ) {
		...
		$this->upload_dir = PrivateSecretStore::directory( 'google-classroom', (int) $this->credential_owner );
		$this->migrate_legacy_secrets();
	...
	private function get_credential_path( $serial = null ) {
		return $this->upload_dir ? $this->upload_dir . DIRECTORY_SEPARATOR . 'credential-' . absint( $serial ? $serial : $this->current_credential ) . '.json' : '';
	}

	private function get_token_path( $serial = null ) {
		return $this->upload_dir ? $this->upload_dir . DIRECTORY_SEPARATOR . 'token-' . absint( $serial ? $serial : $this->current_credential ) . '.json' : '';
	}

	/** Move legacy Google OAuth secrets out of public uploads, once, without exposing their content. */
	private function migrate_legacy_secrets() {
		if ( ! $this->upload_dir || ! $this->credential_owner ) { return; }
		$legacy = trailingslashit( wp_upload_dir()['basedir'] ) . 'tutor-gc/' . absint( $this->credential_owner );
		if ( ! is_dir( $legacy ) || wp_normalize_path( $legacy ) === wp_normalize_path( $this->upload_dir ) ) { return; }
		foreach ( array( 'credential-*.json', 'token-*.json' ) as $pattern ) {
			$files = glob( trailingslashit( $legacy ) . $pattern ); $files = is_array( $files ) ? $files : array();
			foreach ( $files as $source ) {
				if ( ! is_file( $source ) || ! is_readable( $source ) || @filesize( $source ) > 2097152 ) { continue; }
				$data = json_decode( (string) @file_get_contents( $source ), true ); if ( ! is_array( $data ) ) { continue; }
				$dest = trailingslashit( $this->upload_dir ) . basename( $source );
				if ( ! is_file( $dest ) && PrivateSecretStore::write_json( $dest, $data ) ) { @unlink( $source ); }
			}
		}
	}

	private function validate_credential_payload( array $payload ) {
		$config = isset( $payload['web'] ) && is_array( $payload['web'] ) ? $payload['web'] : ( isset( $payload['installed'] ) && is_array( $payload['installed'] ) ? $payload['installed'] : array() );
		if ( empty( $config['client_id'] ) || empty( $config['client_secret'] ) || empty( $config['auth_uri'] ) || empty( $config['token_uri'] ) ) { return false; }
		foreach ( array( 'auth_uri', 'token_uri' ) as $key ) {
			$url = esc_url_raw( (string) $config[ $key ], array( 'https' ) ); $host = strtolower( (string) wp_parse_url( $url, PHP_URL_HOST ) );
			if ( ! $url || ! in_array( $host, array( 'accounts.google.com', 'oauth2.googleapis.com' ), true ) ) { return false; }
		}
		return true;
	}
	...
	public function is_credential_loaded() {
		return $this->credential_path && file_exists( $this->credential_path );
	}
	...
	public function save_credential( $file ) {
		if ( ! $this->upload_dir || empty( $file['tmp_name'] ) || ! is_uploaded_file( $file['tmp_name'] ) || ! empty( $file['error'] ) ) {
			wp_die( esc_html__( 'تعذر حفظ بيانات Google Classroom بشكل آمن.', 'tutor-pro' ) );
		}
		$size = isset( $file['size'] ) ? absint( $file['size'] ) : (int) @filesize( $file['tmp_name'] );
		if ( $size < 2 || $size > 2097152 ) { wp_die( esc_html__( 'ملف بيانات Google Classroom غير صالح.', 'tutor-pro' ) ); }
		$payload = json_decode( (string) file_get_contents( $file['tmp_name'] ), true );
		if ( ! is_array( $payload ) || ! $this->validate_credential_payload( $payload ) ) { wp_die( esc_html__( 'ملف بيانات Google Classroom غير موثوق أو غير صالح.', 'tutor-pro' ) ); }
		$serial = $this->current_credential + 1; $path = $this->get_credential_path( $serial );
		if ( ! $path || ! PrivateSecretStore::write_json( $path, $payload ) ) { wp_die( esc_html__( 'تعذر كتابة بيانات Google Classroom في المخزن الخاص.', 'tutor-pro' ) ); }
		update_user_meta( $this->credential_owner, $this->credential_serial, $serial );
	}

	private function oauth_state_meta_key() { return '_qalam_gc_oauth_state'; }

	private function issue_oauth_state() {
		if ( ! $this->credential_owner ) { return ''; }
		$state = wp_generate_password( 40, false, false );
		update_user_meta( $this->credential_owner, $this->oauth_state_meta_key(), array(
	...
	private function verify_oauth_state() {
		if ( ! $this->credential_owner || get_current_user_id() !== (int) $this->credential_owner ) { return false; }
		$state = sanitize_text_field( (string) Input::get( 'state', '' ) );
		$saved = get_user_meta( $this->credential_owner, $this->oauth_state_meta_key(), true );
		...
		if ( $valid ) { delete_user_meta( $this->credential_owner, $this->oauth_state_meta_key() ); }
		return $valid;
	}

	// Return consent screen URL with a one-time state value bound to the current owner.
	...
	public function assign_token() {
		...
		if ( $this->token_path && file_exists( $this->token_path ) ) {
			$token = PrivateSecretStore::read_json( $this->token_path );
			try {
				$this->client->setAccessToken( $token );
			} catch ( \Throwable $th ) {
				tutor_log( 'Google Classroom token could not be loaded.', $th->getMessage() );
			}
		}
	...
	public function save_token( $code = null, $token = null ) {
		...
		if ( ! $token ) {
			if ( ! $this->verify_oauth_state() ) { throw new \RuntimeException( 'Invalid Google Classroom OAuth state.' ); }
			$token = $this->client->fetchAccessTokenWithAuthCode( sanitize_text_field( (string) $code ) );
			$this->client->setAccessToken( $token );
			$token = $this->client->getAccessToken();
		}

		$path = $this->get_token_path();
		if ( ! $path || ! is_array( $token ) || ! PrivateSecretStore::write_json( $path, $token ) ) {
			throw new \RuntimeException( 'Unable to persist Google Classroom OAuth token securely.' );
		}
	}
	...
	public function get_class_list() {
		...
		$course_list = $this->service_classroom->courses->listCourses( array( 'courseStates' => 'ACTIVE' ) );
	...
	public function get_stream( $local_id, $token = null, $only_stream = false ) {
		...
		! is_array( $announcements ) ? $announcements = array() : 0;
	...
	private function import_course_post( $course ) {
		...
		update_post_meta( $post_id, $this->gc_post_time, gmdate( 'Y-m-d h:i a', tutor_time() ) );
