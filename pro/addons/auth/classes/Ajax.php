...
class Ajax {
	...
	public function resent_login_otp() {
		...
		if ( ! isset( $otp_time ) || ! isset( $otp_data->user ) || ( isset( $otp_data->method ) && 'email' !== $otp_data->method ) || OtpSecurity::expired( $otp_data ) ) {
			wp_send_json_error(
		...
		// Update session OTP.
		$otp_data->code_hash = OtpSecurity::hash_code( (string) $new_otp );
		$otp_data->expires_at = time() + OtpSecurity::TTL;
		$otp_data->attempts = 0;
