...
$method = isset( $opt_data->method ) ? (string) $opt_data->method : 'email';
	...
	<div style="max-width:450px;margin: 0 auto;margin-top:80px;padding: 0 24px;">
		<form method="post" id="tutor-otp-form">
			...
			<p class="tutor-mt-16 tutor-mb-20">
				<?php if ( 'totp' === $method ) : ?>
					<?php esc_html_e( 'Enter the 6-digit code from your authenticator app or a recovery code.', 'tutor-pro' ); ?>
				<?php else : ?>
					<?php echo esc_html( sprintf( __( 'We have sent an e-mail to your registered e-mail address (%s) with an OTP code.', 'tutor-pro' ), $email_hint ) ); ?>
					<?php esc_html_e( 'Please collect OTP and enter here to complete login process.', 'tutor-pro' ); ?>
				<?php endif; ?>
			</p>
			<div class="tutor-d-flex">
				<input required="required" 
						name="tutor-login-otp" class="tutor-form-control" 
						type="text" inputmode="numeric" autocomplete="one-time-code" maxlength="16"
						autofocus
...
<script>
	document.addEventListener('DOMContentLoaded', async function() {
		...
		// Init resend only for email challenges.
		if ('<?php echo esc_js( $method ); ?>' === 'email') { runCounter(); btnOtp.click(handleResentOtpAjax); }
