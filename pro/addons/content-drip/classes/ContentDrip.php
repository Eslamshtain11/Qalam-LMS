...
class ContentDrip {
	...
	public function is_lock_lesson( $content_id ) {
		...
		if ( 'unlock_by_date' === $drip_type ) {
			$unlock_timestamp = strtotime( (string) get_item_content_drip_settings( $content_id, 'unlock_date' ) );
			if ( $unlock_timestamp ) {
			...
			$this->unlock_message = __( 'This content is locked because its drip date is missing or invalid.', 'tutor-pro' );
			return true;
			...
			$this->unlock_message = __( 'This content is locked because the drip day configuration is missing or invalid.', 'tutor-pro' );
			return true;
		}
			...
			if ( tutor_utils()->count( $this->prerequisites ) ) {
				...
				foreach ( $this->prerequisites as $id ) {
					...
					if ( ! $item || ! is_object( $item ) ) {
						$required_finish[] = esc_html__( 'A configured prerequisite is missing or unavailable.', 'tutor-pro' );
						continue;
					}
				...
				$this->unlock_message = __( 'This content is locked because its prerequisite configuration is empty.', 'tutor-pro' );
				return true;
			}
		} elseif ( ! in_array( $drip_type, array( 'unlock_sequentially' ), true ) ) {
			$this->unlock_message = __( 'This content is locked because its drip configuration is invalid.', 'tutor-pro' );
			return true;
		}
	...
	private function get_mailing_courses() {
		...
		$courses = array_map(
			function ( $element ) {
				$element->meta_value = unserialize( $element->meta_value, array( 'allowed_classes' => false ) );
				return $element;
			},
			$courses
		);
