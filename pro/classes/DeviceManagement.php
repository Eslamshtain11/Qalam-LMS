...
class DeviceManagement {
	...
	public static function get_current_user_location( $user_ip = '' ) {
		...
		// Never leak private/invalid addresses, and never bypass WordPress HTTP safety.
		if ( ! filter_var( $user_ip, FILTER_VALIDATE_IP, FILTER_FLAG_NO_PRIV_RANGE | FILTER_FLAG_NO_RES_RANGE ) ) {
			return $location;
		}

		$response = wp_safe_remote_get(
			'https://ipinfo.io/' . rawurlencode( $user_ip ) . '/json',
			array(
				'timeout'     => 3,
				'redirection' => 0,
			)
		...
		$details = json_decode( (string) wp_remote_retrieve_body( $response ) );
		if ( is_object( $details ) ) {
			$location['country'] = sanitize_text_field( (string) ( $details->country ?? '' ) );
			$location['city']    = sanitize_text_field( (string) ( $details->city ?? '' ) );
		}
	...
	public static function remove_device_manually() {
		...
		$table     = $wpdb->usermeta;
		$umeta_id  = Input::post( 'umeta_id', 0, Input::TYPE_INT );
		$current_id = get_current_user_id();

		if ( $umeta_id < 1 || $current_id < 1 ) {
			wp_send_json_error( $response, 400 );
		}

		$meta = $wpdb->get_row(
			$wpdb->prepare(
				"SELECT umeta_id, user_id, meta_key FROM {$wpdb->usermeta} WHERE umeta_id = %d AND user_id = %d LIMIT 1",
				$umeta_id,
				$current_id
			)
		);

		// This endpoint is intentionally self-service only. Admins have a separate capability-gated handler.
		if ( $meta && 0 === strpos( (string) $meta->meta_key, self::LOGIN_INFO_KEY ) ) {
			$where  = array( 'umeta_id' => $umeta_id, 'user_id' => $current_id, 'meta_key' => $meta->meta_key );
