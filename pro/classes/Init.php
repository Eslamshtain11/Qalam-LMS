...
class Init {
	...
	private $instructor;
	private $rest_api;
	private $qalam_constructors_loaded = false;
	...
	public function __construct() {
		...
		/*
		 * Qalam load-order bridge: qalam-lms-pro may be loaded by WordPress before
		 * qalam-lms. Never decide that Core is missing during the Pro file include.
		 * Wait for tutor_loaded, or boot immediately if Core already fired it.
		 */
		if ( did_action( 'tutor_loaded' ) ) {
			$this->qalam_boot_after_core();
		} else {
			add_action( 'tutor_loaded', array( $this, 'qalam_boot_after_core' ), 1 );
		}
		add_action( 'admin_notices', array( $this, 'qalam_maybe_required_version_notice' ), 1 );
		add_action( 'admin_notices', array( $this, 'qalam_maybe_boot_error_notice' ), 2 );
	...
	/** Boot Pro only after Qalam Core has completed its runtime bootstrap. */
	public function qalam_boot_after_core(): void {
		if ( $this->qalam_constructors_loaded ) {
			return;
		}
		if ( ! defined( 'TUTOR_VERSION' ) || ! defined( 'TUTOR_FILE' ) || ! function_exists( 'tutor_lms' ) || ! $this->is_tutor_core_has_req_verion() ) {
			return;
		}

		// Never run the full Pro runtime inside WordPress' activation sandbox.
		if ( defined( 'WP_SANDBOX_SCRAPING' ) && WP_SANDBOX_SCRAPING ) {
			return;
		}

		$this->qalam_constructors_loaded = true;
		try {
			$this->load_constructors_asset();
			delete_option( 'qalam_lms_pro_boot_error' );
		} catch ( \Throwable $e ) {
			$this->qalam_constructors_loaded = false;
			update_option(
				'qalam_lms_pro_boot_error',
				array(
	...
	/** Show dependency notice only after all plugins have had a chance to load. */
	public function qalam_maybe_required_version_notice(): void {
		if ( ! defined( 'TUTOR_VERSION' ) || ! defined( 'TUTOR_FILE' ) || ! function_exists( 'tutor_lms' ) || ! $this->is_tutor_core_has_req_verion() ) {
			$this->required_version_notice();
		}
	}

	/** Show a recoverable diagnostic instead of taking the whole admin down. */
	public function qalam_maybe_boot_error_notice(): void {
		$error = get_option( 'qalam_lms_pro_boot_error' );
		if ( ! is_array( $error ) || empty( $error['message'] ) ) {
			return;
		}
		printf(
			'<div class="notice notice-error"><p><strong>Qalam LMS Pro — خطأ تحميل:</strong> %s <code>%s:%d</code></p></div>',
	...
	public function load_constructors_asset() {
		...
		// Qalam security quarantine: Tutor commercial license gating/updater is disabled.
		// Qalam Cloud licensing will be introduced only after parity/branding gates.
	...
	public function load_addons() {
		...
		if ( count( $addons_dir ) > 0 ) {
			foreach ( $addons_dir as $key => $value ) {
				...
				// Qalam feature gate: a SaaS-denied capability never boots. A locally disabled
				// capability is only primed on the Add-ons/AJAX control path so its manifest
				// and enable hooks remain available; everywhere else it disappears at runtime.
				if ( function_exists( 'qalam_190_packaged_runtime_should_boot' ) && ! qalam_190_packaged_runtime_should_boot( $addon_dir_name ) ) {
					continue;
				}
				...
				try {
					...
					$addon_errors[ $addon_dir_name ] = array(
						'message' => $e->getMessage(),
						'file'    => $e->getFile(),
						'line'    => $e->getLine(),
					);
		...
		if ( $addon_errors ) {
			update_option( 'qalam_lms_pro_addon_boot_errors', $addon_errors, false );
		} else {
			delete_option( 'qalam_lms_pro_addon_boot_errors' );
		}
	}
	...
	public function update_assignment_meta( $assignments ): bool {
		foreach ( $assignments as $assignment ) {
			...
			$assignment_info = unserialize( $assignment->meta_value, array( 'allowed_classes' => false ) );
	...
	public function is_tutor_core_has_req_verion() {
		if ( ! defined( 'TUTOR_VERSION' ) || ! defined( 'TUTOR_FILE' ) || ! file_exists( TUTOR_FILE ) ) {
			return false;
		}
		return version_compare( TUTOR_VERSION, TUTOR_CORE_REQ_VERSION, '>=' );
	}
	...
	public function tutor_core_version_compare() {
		if ( defined( 'TUTOR_VERSION' ) && ! $this->is_tutor_core_has_req_verion() ) {
	...
	public function required_version_notice() {
		?>
		<div class="notice notice-error tutor-install-notice" style="display:flex;justify-content:space-between;align-items:center;direction:rtl;text-align:right;">
			<div class="tutor-install-notice-inner" style="display:flex;column-gap:20px;align-items:center;">
				<div class="tutor-install-notice-icon"><img style="width:72px;height:auto" src="<?php echo esc_url( tutor_pro()->url . 'assets/images/qalam-logo.svg' ); ?>" alt="Qalam LMS"></div>
				<div class="tutor-install-notice-content">
					<h2><?php esc_html_e( 'يلزم تفعيل Qalam LMS Core أولًا.', 'tutor-pro' ); ?></h2>
					<p><?php esc_html_e( 'هذه الحزمة هي الميزات المتقدمة لقلم، وتحتاج إلى Qalam LMS Core بنفس الإصدار لتعمل كل الوظائف والملحقات بصورة صحيحة.', 'tutor-pro' ); ?></p>
				</div>
			</div>
		</div>
		<?php
	...
	public function init_license() {
		// Intentionally disabled by the Qalam security quarantine.
		// Never load the donor commercial updater/license runtime.
	}
