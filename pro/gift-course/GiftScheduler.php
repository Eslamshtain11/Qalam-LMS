...
class GiftScheduler {
	...
	public function schedule( object $payload, string $status = self::STATUS_SCHEDULED ) {
		// Qalam hardening: reference_id is the idempotency key. Replayed order events update the same schedule instead of duplicating the gift.
		if ( ! empty( $payload->reference_id ) ) {
			$existing = $this->get( array( 'reference_id' => (string) $payload->reference_id ) );
			if ( $existing && ! empty( $existing->id ) ) {
				$this->update( array( 'payload'=>wp_json_encode($payload,JSON_UNESCAPED_UNICODE),'status'=>$status,'scheduled_at_gmt'=>$payload->scheduled_at_gmt ), array( 'id'=>(int)$existing->id ) );
