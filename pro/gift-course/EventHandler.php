...
class EventHandler {
	...
	public function __construct() {
		...
		add_action( 'tutor_order_placed', array( $this, 'handle_gift_course_after_native_order' ) );
	}
	...
	public function handle_order_status_update( $order_id, $status_from, $status_to ) {
		// Cancellation/refund events must revoke only the entitlement bound to this gift reference.
		if ( in_array( (string) $status_to, array( 'cancelled','canceled','refunded','failed','void','unpaid' ), true ) ) { $this->revoke_gifts_for_order( $order_id, (string)$status_to ); return; }
		// For native status_to is payment status.
		$is_tutor_monetization = tutor_utils()->is_monetize_by_tutor();
		if ( $is_tutor_monetization && 'paid' !== $status_to ) { return; }
	...
	/** Revoke only the entitlement created by the exact gift reference. Idempotent by design. */
	private function revoke_gifts_for_order( $order_id, $reason = 'cancelled' ) {
		try { $proceeder=GiftCourse::get_gift_proceeder(); $order=$proceeder->get_order((int)$order_id); if(!$order||empty($order->items))return; }
		catch( \Throwable $e ){ tutor_log($e); return; }
		foreach($order->items as $item){
			try{$gift=$proceeder->get_gift_data($item);if(!$gift||empty($gift->reference_id))continue;$ref=sanitize_text_field((string)$gift->reference_id);(new GiftScheduler())->delete(array('reference_id'=>$ref));
				$email=sanitize_email((string)($gift->recipient_email??''));$course=absint($gift->course_id??0);$user=$email?get_user_by('email',$email):false;
				if($user&&$course){$en=tutor_utils()->get_enrolled_data($user->ID,$course);if($en&&'completed'===$en->post_status&&hash_equals($ref,(string)get_post_meta($en->ID,'_qalam_gift_reference',true))){wp_update_post(array('ID'=>(int)$en->ID,'post_status'=>'cancel'));update_post_meta((int)$en->ID,'_qalam_gift_revoked_reason',sanitize_text_field((string)$reason));}}
				$proceeder->update_item_meta($gift->order_item_id??($item->primary_id??0),GiftCourse::GIFT_STATUS_META,'revoked');
			}catch(\Throwable $e){tutor_log($e);}
		}
	}
	...
	/**
	 * Process gift course if the order is paid while creation
	 * this is happen when user have 100% discount
	 *
	 * @since 4.0.0
	 *
	 ...
	 * @return void
	 */
	public function handle_gift_course_after_native_order( array $order_data ) {
		$payment_status = $order_data['payment_status'] ?? '';
		$order_type     = $order_data['order_type'] ?? '';

		if ( ! $payment_status || ! $order_type ) {
			return;
		}

		if ( OrderModel::PAYMENT_PAID === $payment_status && OrderModel::TYPE_SINGLE_ORDER === $order_type ) {
			// May be gift item.
			$this->handle_order_status_update( $order_data['id'], OrderModel::PAYMENT_PENDING, OrderModel::PAYMENT_PAID );
		}
	}
}
