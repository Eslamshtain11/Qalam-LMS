...
class GiftEnrollment {
	...
	public function ajax_gift_enrollment() {
		...
		if ( $enroll ) {
			// Bind this entitlement to the exact gift reference so cancellation/refund cannot revoke unrelated enrollment.
			update_post_meta( (int) $enroll, '_qalam_gift_reference', sanitize_text_field( (string) $reference_id ) );
			update_post_meta( (int) $enroll, '_qalam_gift_order_id', absint( $can_claim->payload->order_id ?? 0 ) );
			update_post_meta( (int) $enroll, '_qalam_gift_order_item_id', absint( $can_claim->payload->order_item_id ?? 0 ) );
	...
	public function claim_gift() {
		...
		// Redirect to Qalam LMS Dashboard.
