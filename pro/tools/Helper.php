...
class Helper {
	...
	public static function upload_file_by_url( $file_url ) {
		...
		$file_name = sanitize_file_name( wp_basename( (string) ( $parts['path'] ?? '' ) ) );
		$file_type = wp_check_filetype( $file_name, get_allowed_mime_types() );
		if ( '' === $file_name || empty( $file_type['ext'] ) || empty( $file_type['type'] ) ) {
			return new \WP_Error( 'invalid_file_type', 'Remote file type is not allowed.' );
		}
		...
		$target_dir = wp_normalize_path( (string) $upload['path'] );
		if ( ! wp_mkdir_p( $target_dir ) ) {
			return new \WP_Error( 'upload_dir_error', 'Unable to prepare uploads directory.' );
		}
		$file_name = wp_unique_filename( $target_dir, $file_name );
		$file_path = $target_dir . '/' . $file_name;

		$response = wp_safe_remote_get( $file_url, array( 'timeout' => 120, 'redirection' => 2, 'sslverify' => true ) );
		if ( is_wp_error( $response ) ) {
			return $response;
		}
		if ( 200 !== (int) wp_remote_retrieve_response_code( $response ) ) {
			return new \WP_Error( 'download_failed', 'Remote server did not return a successful response.' );
		}
		$file_data = wp_remote_retrieve_body( $response );
		if ( '' === $file_data || false === @file_put_contents( $file_path, $file_data, LOCK_EX ) ) {
			return new \WP_Error( 'download_failed', 'Failed to save remote content.' );
		}
		@chmod( $file_path, 0644 );

		$checked = wp_check_filetype_and_ext( $file_path, $file_name, get_allowed_mime_types() );
		if ( ! empty( $checked['ext'] ) && ! empty( $checked['type'] ) ) {
			$file_type = $checked;
		}
		if ( empty( $file_type['type'] ) ) {
			@unlink( $file_path );
			return new \WP_Error( 'invalid_file_type', 'Downloaded file type is not allowed.' );
		}

		$file_url_local = trailingslashit( (string) $upload['url'] ) . rawurlencode( $file_name );
