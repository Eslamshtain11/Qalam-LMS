...
class Paypal extends BasePayment {
	...
	public function verifyAndCreateOrderData( object $payload ): object {
		try {
			...
			$event_id       = is_object( $payload_stream ) ? sanitize_text_field( (string) ( $payload_stream->id ?? '' ) ) : '';
			$replay_key     = $event_id ? 'qalam_paypal_webhook_' . hash( 'sha256', $event_id ) : '';

			if ( $replay_key && get_transient( $replay_key ) ) {
			...
			switch ( $payload_stream->event_type ) {
				case self::CHECKOUT_ORDER_APPROVED:
					...
					if ( $replay_key ) { set_transient( $replay_key, 1, 7 * DAY_IN_SECONDS ); }
					return new \stdClass();

				case self::PAYMENT_CAPTURE_COMPLETED:
					$return_data = $this->setReturnData( $payload_stream );
					if ( $replay_key ) { set_transient( $replay_key, 1, 7 * DAY_IN_SECONDS ); }
					return $return_data;
				...
				case self::PAYMENT_CAPTURE_REFUNDED:
					...
					return new \stdClass();
				default:
					if ( $replay_key ) { set_transient( $replay_key, 1, DAY_IN_SECONDS ); }
					return new \stdClass();
			}
		} catch ( RequestException $error ) {
