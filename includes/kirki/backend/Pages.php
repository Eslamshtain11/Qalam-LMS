...
class Pages
{
    ...
    public function tde_get_apis()
    {
        ...
        if ( ! current_user_can( 'edit_tutor_courses' ) && ! current_user_can( 'manage_options' ) ) {
            wp_send_json_error( array( 'message' => __( 'Access Denied', 'tutor' ) ), 403 );
        }
    ...
    private function generate_utility_page_content_with_fullcanvas_template_using_type($type, $template_path)
    {
        ...
        if ($utility_page) {
            $html = apply_filters(
                'kirki_html_generator',
                '',
                $utility_page['id']
            );
            $custom_data = [
                'kirki_template_content' => $html,
                'kirki_template_id'      => $utility_page['id'],
            ];
            set_query_var('kirki_custom_data', $custom_data);
            $template_path = KIRKI_FULL_CANVAS_TEMPLATE_PATH;
        }
    ...
    {
        $template = apply_filters('kirki_template_finder', 'post', get_post(get_the_ID()));
        if ($template) {
            ...
            if ($course_template->post_status === 'publish') {
                ...
                $custom_data = [
                    'kirki_template_content' => $html, // Example: Get the current post ID
    ...
    {
        wp_enqueue_script(TDE_APP_PREFIX . '-tutor-kirki-preview', TDE_PLUGIN_ROOT_BASE . 'build/js/preview.min.js', ['wp-i18n'], TDE_APP_VERSION, true);
        wp_enqueue_style(TDE_APP_PREFIX . '-tutor-kirki-preview', TDE_PLUGIN_ROOT_BASE . 'build/css/preview.min.css', null, TDE_APP_VERSION);
        wp_localize_script(TDE_APP_PREFIX . '-tutor-kirki-preview', TDE_APP_PREFIX, [
    ...
    private function is_cart_page()
    {
        ...
        if (isset($wp_query->query['kirki_utility_page_type']) && $wp_query->query['kirki_utility_page_type'] === 'lms_cart') {
            return true;
        }
        return false;
    }
