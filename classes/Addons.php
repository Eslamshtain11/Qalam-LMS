...
class Addons {
	...
	public function prepare_addons_data() {
		...
		if ( is_array( $addons ) && count( $addons ) ) {
			foreach ( $addons as $base_name => $addon ) {
				...
				$thumbnail_url = tutor()->url . 'assets/images/qalam-logo.svg';
		...
		foreach ( $free_addon_list as $addon_name => $addon ) {
			$suffix = "/addons/{$addon_name}/{$addon_name}.php";
			foreach ( $plugins_data as $plugin_key => $plugin_data ) {
				$key = '/' . ltrim( str_replace( '\\', '/', (string) $plugin_key ), '/' );
				if ( str_ends_with( $key, $suffix ) ) {
					$prepared_addons[] = $plugin_data;
					break;
				}
			}
		}

		// Qalam safety net: if the preferred ordering list changes upstream, display all
		// registered Pro add-ons instead of rendering a blank marketplace.
		if ( empty( $prepared_addons ) && ! empty( $plugins_data ) ) {
			$prepared_addons = array_values( $plugins_data );
		}
	...
	public function addon_enable_disable() {
		...
		foreach ( $form_data as $addon_field_name => $is_enable ) {
			$legacy_field_name = preg_replace( '#^[^/]+/addons/#', 'tutor-pro/addons/', (string) $addon_field_name );
			...
			if ( $before_status !== $after_status ) {
				...
				if ( $is_enable ) {
					...
					if ( $legacy_field_name !== $addon_field_name ) { do_action( "tutor_addon_before_enable_{$legacy_field_name}" ); }
					...
					if ( $legacy_field_name !== $addon_field_name ) { do_action( "tutor_addon_after_enable_{$legacy_field_name}" ); }
				} else {
					do_action( "tutor_addon_before_disable_{$addon_field_name}" );
					if ( $legacy_field_name !== $addon_field_name ) { do_action( "tutor_addon_before_disable_{$legacy_field_name}" ); }
					...
					if ( $legacy_field_name !== $addon_field_name ) { do_action( "tutor_addon_after_disable_{$legacy_field_name}" ); }
				}
	...
	public function addons_lists_to_show() {
		$addons = array(
			...
			'google-classroom'         => array(
				...
				'description' => __( 'Enable to integrate Qalam LMS with Google Classroom.', 'tutor' ),
			),
			'tutor-zoom'               => array(
				'name'        => __( 'Tutor Zoom Integration', 'tutor' ),
				'description' => __( 'Connect Qalam LMS with Zoom to host live online classes.', 'tutor' ),
			),
			...
			'buddypress'               => array(
				...
				'description' => __( 'Boost engagement with social features through BuddyPress for Qalam LMS.', 'tutor' ),
			),
