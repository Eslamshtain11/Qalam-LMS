...
class Utils {
	...
	public function check_current_user_capability( string $capability = 'manage_options', int $object_id = 0 ) {
		// Qalam platform owners/managers are LMS administrators without receiving
		// WordPress manage_options. Keep the bridge inside Tutor's own capability
		// helper so external WordPress administration remains inaccessible.
		$can = ( 'manage_options' === $capability && User::is_admin() )
			|| ( $object_id ? current_user_can( $capability, $object_id ) : current_user_can( $capability ) );
		if ( ! $can ) {
	...
	public function get_human_readable_time( $from, $to = null, $format = null, $show_postfix_text = true ) {
		...
		$from_date_time = new \DateTime( $from, $wp_tz );
		$to_date_time   = null === $to ? new \DateTime( 'now', $wp_tz ) : new \DateTime( $to, $wp_tz );
		$format         = null === $format ? '%ad %hh %im %ss' : $format;
		...
		$time_span    = $to_date_time->diff( $from_date_time );
		$postfix_text = true === $show_postfix_text ? $postfix_text : '';

		return $time_span->format( $format ) . $postfix_text;
	}
	...
	public function get_addon_config( $addon_field = null ) {
		...
		$addons_config = is_array( $addons_config ) ? $addons_config : array();
		$pro_root = defined( 'TUTOR_PRO_FILE' ) ? dirname( plugin_basename( TUTOR_PRO_FILE ) ) : 'tutor-pro';
		$normalized = str_replace( '\\', '/', (string) $addon_field );
		$candidates = array();
		if ( false !== strpos( $normalized, '/addons/' ) ) {
			$suffix = substr( $normalized, strpos( $normalized, '/addons/' ) );
			$candidates[] = $pro_root . $suffix;
			$candidates[] = 'qalam-lms-pro' . $suffix;
			$candidates[] = 'tutor-pro' . $suffix;
			$candidates[] = $normalized;
		} else {
			$slug = trim( $normalized, '/' );
			$candidates[] = "{$pro_root}/addons/{$slug}/{$slug}.php";
			$candidates[] = "qalam-lms-pro/addons/{$slug}/{$slug}.php";
			$candidates[] = "tutor-pro/addons/{$slug}/{$slug}.php";
		}
		foreach ( array_unique( $candidates ) as $candidate ) {
			if ( isset( $addons_config[ $candidate ] ) ) {
				return $addons_config[ $candidate ];
			}
		}
		return false;
	}
	...
	public function can_user_manage( $content, $object_id, $user_id = 0, $allow_current_admin = true ) {
		...
		if ( $course_id ) {
			if ( $allow_current_admin && User::is_admin( $user_id ) ) {
	...
	public function prepare_free_addons_data() {
		...
		if ( is_array( $addons ) && count( $addons ) ) {
			foreach ( $addons as $base_name => $addon ) {
				...
				// Add add-on enable status using the active Pro folder, not a hard-coded tutor-pro path.
				$plugins_data[ $base_name ]['base_name'] = $base_name;
				$config = $this->get_addon_config( $base_name );
				$plugins_data[ $base_name ]['is_enabled'] = $has_pro && is_array( $config ) ? (int) ( $config['is_enable'] ?? 0 ) : 0;
			}
		}
	...
	public function tutor_custom_header() {
		if ( function_exists( 'qalam_240_render_shell_header' ) && function_exists( 'qalam_240_is_public_surface' ) && qalam_240_is_public_surface() && ( ! function_exists( 'qalam_210_is_dashboard_request' ) || ! qalam_210_is_dashboard_request() ) ) {
			if ( function_exists( 'qalam_240_is_login_surface' ) && qalam_240_is_login_surface() ) {
				// Standalone Qalam login owns its own document shell.
			} else {
	...
	public function tutor_custom_footer() {
		if ( function_exists( 'qalam_240_render_shell_footer' ) && function_exists( 'qalam_240_is_public_surface' ) && qalam_240_is_public_surface() ) {
			if ( function_exists( 'qalam_240_is_login_surface' ) && qalam_240_is_login_surface() ) {
				// Standalone Qalam login owns its own document shell.
			} else {
