...
class SampleCourse {
	...
	private function upload_file_by_url( $file_url ) {
		...
		if ( '' === $file_name || empty( $file_type['type'] ) ) {
		...
		$response = wp_safe_remote_get( $file_url, array( 'timeout' => 300, 'redirection' => 2, 'sslverify' => true ) );
		if ( is_wp_error( $response ) || 200 !== (int) wp_remote_retrieve_response_code( $response ) ) {
			return is_wp_error( $response ) ? $response : new \WP_Error( 'download_failed', 'Failed to download content.' );
		}
		$file_data = wp_remote_retrieve_body( $response );
		if ( '' === $file_data || false === @file_put_contents( $file_path, $file_data, LOCK_EX ) ) {
			return new \WP_Error( 'download_failed', 'Failed to save downloaded content.' );
		}
		...
		$attachment_args = array(
			'guid'           => trailingslashit( (string) $upload['url'] ) . rawurlencode( $file_name ),
