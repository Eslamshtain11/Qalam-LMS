...
class Course extends Tutor_Base {
	...
	public function validate_video_source( $params, &$errors ) {
		if ( isset( $params['video'] ) ) {
			$video_source_type = isset( $params['video']['source'] ) ? $params['video']['source'] : '';
			if ( tutor_is_rest() ) {
	...
	public function localize_course_builder_data( $data ) {
		...
		$is_frontend_builder = tutor_utils()->is_tutor_frontend_dashboard( 'create-course' );
		$frontend_edit       = $is_frontend_builder && $course_id;
		$is_qalam_builder    = function_exists( 'qalam_210_is_course_builder_request' ) && qalam_210_is_course_builder_request();

		if ( ! $backend_edit && ! $frontend_edit && ! $is_qalam_builder ) {
			return $data;
		}
		...
		$is_qalam_managed = function_exists( 'qalam_210_user_is_managed' ) && qalam_210_user_is_managed();
		if ( $is_qalam_managed && function_exists( 'qalam_210_dashboard_url' ) ) {
			$dashboard_url = qalam_210_dashboard_url();
		...
		if ( $is_qalam_managed && function_exists( 'qalam_210_dashboard_url' ) ) {
			...
			$data['frontend_course_list_url'] = qalam_210_dashboard_url( 'courses' );
		} else {
