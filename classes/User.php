...
class User {
	...
	public static function has_any_role( array $roles, $user_id = 0 ) {
		...
		// Qalam platform owners/managers are LMS administrators only. They deliberately
		// do not receive the WordPress administrator role or dangerous WP capabilities.
		if ( in_array( self::ADMIN, $roles, true ) && array_intersect( array( 'qalam_owner', 'qalam_manager' ), (array) $user->roles ) ) {
			return true;
		}
	...
	public static function is_admin( $user_id = 0 ) {
		...
		return $user && (bool) array_intersect( array( 'qalam_owner', 'qalam_manager' ), (array) $user->roles );
	}
