...
class Quiz {
	...
	/**
	 * Skip loading templates that are not packaged with core Qalam LMS.
	 *
	 * @since 4.0.0
	 *
	...
	public function tutor_instructor_feedback() {
		...
		if ( $attempt_info ) {
			...
			$unserialized = unserialize( $attempt_details->attempt_info, array( 'allowed_classes' => false ) );
	...
	public static function tutor_quiz_attempt_submit() {
		...
		// Check & die if don't have quiz access.
		$quiz_id = Input::post( 'quiz_id', 0, Input::TYPE_INT );
		if ( $quiz_id < 1 || (int) $attempt->quiz_id !== (int) $quiz_id ) {
			die( 'Operation not allowed, quiz mismatch' );
		}
		QuizModel::has_quiz_access( $attempt->quiz_id );
		...
		$attempt_answers = apply_filters( 'qalam_quiz_attempt_answers_before_submit', $attempt_answers, $attempt, $attempt_id, $user_id );
	...
	public static function manage_attempt_answers( $attempt_answers, $attempt, $attempt_id, $course_id, $user_id ) {
		...
		foreach ( $attempt_answers as $posted_attempt_id => $attempt_answer ) {
			$posted_attempt_id = absint( $posted_attempt_id );
			if ( $posted_attempt_id !== (int) $attempt_id || ! self::validate_attempt( $posted_attempt_id, $user_id ) ) {
				continue;
			}
			...
			if ( $question_ids ) {
				$in_clause = QueryHelper::prepare_in_clause( $question_ids );
				$question_ids = array_map( 'absint', (array) $wpdb->get_col( $wpdb->prepare( "SELECT question_id FROM {$wpdb->prefix}tutor_quiz_questions WHERE quiz_id = %d AND question_id IN({$in_clause})", (int) $attempt->quiz_id ) ) ); // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
			}
			...
			if ( tutor_utils()->count( $quiz_answers ) ) {
				...
				foreach ( $quiz_answers as $question_id => $answers ) {
					...
					if ( ! $question_id || ! is_object( $question ) || (int) ( $question->quiz_id ?? 0 ) !== (int) $attempt->quiz_id || ! in_array( $question_id, $question_ids, true ) ) {
						continue;
					}
					$valid_answered_count++;
			...
			$attempt_info = array(
				'total_answered_questions' => $valid_answered_count,
